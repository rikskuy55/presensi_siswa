...
@section('judulkonten', 'Data Jadwal Mengajar')

@section('isikonten')
<div class="text-center">
    <h1>Daftar Jadwal Mengajar</h1>
</div>

    <div class="d-flex justify-content-end mt-3">
        <a href="{{ route('jadwal-mapel.create') }}" class="btn btn-success btn-round me-3">
            ...
            <tbody>
                @foreach ($jadwalMapels as $jadwalMapel)
                    <tr>
                        ...
                        <td>{{ $jadwalMapel->kelas ? $jadwalMapel->kelas->nama_kelas : 'Kelas tidak ditemukan' }}</td>
                        <td>{{ $jadwalMapel->guruMapel->guru->nama_guru }}</td>
                        <td>{{ $jadwalMapel->guruMapel->mataPelajaran->nama_mata_pelajaran }}</td>
                        <td>{{ $jadwalMapel->hari }}</td>
                        <td>{{ $jadwalMapel->jam_mulai }}</td>
                        <td>{{ $jadwalMapel->jam_selesai }}</td>
                        <td class="text-center">
                            <a href="{{ route('jadwal-mapel.edit', $jadwalMapel->id) }}" class="btn btn-primary">
                                <i class="fas fa-edit"></i>
                            </a>

                            <form action="{{ route('jadwal-mapel.destroy', $jadwalMapel->id) }}" method="POST"
        ...
        $(document).ready(function() {
            ...
            $('.form-hapus').on('submit', function(e) {
                ...
                Swal.fire({
                    ...
                    text: 'Jadwal Mengajar akan dihapus secara permanen!',
                    icon: 'warning',
