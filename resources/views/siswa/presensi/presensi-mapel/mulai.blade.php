...
@section('title', 'Absensi Mata Pelajaran - Mulai')
...
<div class="appHeader bg-primary text-light">
    ...
    <div class="pageTitle">Absen Mata Pelajaran {{ $mataPelajaran->nama_mata_pelajaran }}</div>
    <div class="right"></div>
</div>
<!-- * App Header -->
    ...
    <div class="container mt-5">
        <div class="row" style="margin-top: 70px; padding: 16px;">
            <div class="col text-center">
                <div id="currentDateTime" class="alert" style="background-color: #00bcd4; color: white; font-size: 16px; font-weight: bold; border-radius: 5px; padding: 10px;">
                    <!-- Tanggal, Hari, Jam -->
                </div>
                
                <!-- Menampilkan informasi jam masuk dari jadwal -->
                <div class="alert alert-warning">
                    <ion-icon name="time-outline" style="font-size: 20px;"></ion-icon>
                    <span style="color: black; font-size: 16px;">
                        Jam mulai: <strong>{{ $jadwal->jam_mulai }}</strong><br> 
                        Pastikan Anda melakukan absen sebelum waktu ini. Kehadiran terlambat akan tercatat.
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-12 col-md-6">
                <button id="takeabsen" class="btn btn-primary btn-block d-flex justify-content-center align-items-center py-2" style="border-radius: 50px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
                    <ion-icon name="camera-outline" style="font-size: 24px;"></ion-icon>
                    <span class="ml-2" style="font-size: 18px;">Absen Mulai</span>
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        document.getElementById("takeabsen").addEventListener("click", function() {
            Webcam.snap(function(data_uri) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('siswa.submit-absensi-mapel') }}", // Update dengan route yang sesuai untuk absensi mata pelajaran
                    data: {
                        _token: "{{ csrf_token() }}",
                        image: image,
                        lokasi: lokasi.value,
                        mapel_id: "{{ $mapel_id }}", // Kirim ID mata pelajaran
                    },
                    cache: false,
                    success: function(response) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                window.location.href = '/dashboard'; // Atur redirect sesuai kebutuhan
                            }
                        });
                    },
                    error: function(xhr) {
